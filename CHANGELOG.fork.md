...
- The agents panel gained a third `folders` ordering that mirrors the spaces panel's organization: agents nest under their space and folder, worktree families keep their nesting, and each space's agent list can be collapsed independently. With several machines connected, each machine's agents form their own headed section with that machine's folders and collapse state. The choice persists like the existing orderings and can be set as the startup default.
