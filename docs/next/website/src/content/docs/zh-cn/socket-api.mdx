...
| 文件夹 | `folder.create`、`folder.list`、`folder.rename`、`folder.assign`、`folder.move`、`folder.delete` |
...
文件夹方法用于整理侧边栏中空间的顺序。服务器拥有一个文件夹与松散工作区交错排列的顶层有序序列，以及每个文件夹的有序成员列表。所有读取都反映这一规范顺序：`folder.list` 返回文件夹和顶层顺序，工作区列表遵循展平后的顺序，工作区位于文件夹内时其记录带有 `folder_id`（松散工作区省略该字段）。

`herdr folder` CLI 命令组为 shell 和智能体 1:1 封装了这六个方法；参见 [CLI 参考](/docs/cli-reference/)。
...
`folder.create` 返回新文件夹的 id。名称必须包含可见文本；由于 id 才是身份标识，允许重复名称。

`folder.assign` 把一个工作区移入文件夹，或在 `folder_id` 为 `null` 时移回顶层。指派 worktree 家族中的任一成员会移动整个家族，响应和事件会报告所有受影响的工作区。可选的 `position` 是受影响块在目标容器中的最终索引：目标为文件夹的成员列表，或在 `folder_id` 为 `null` 时为顶层顺序（一个文件夹算作一个条目）。省略则追加到末尾；超出范围则截断到末尾。带位置地指派到当前容器就是在其内部的普通重排：同一文件夹内的定位指派发出 `folder.updated`，而 `folder_id: null` 加位置的顶层指派则是顶层重排。

`folder.move` 在顶层顺序中重新定位文件夹并返回实际位置。`folder.delete` 把成员按原有相对顺序释放回文件夹原来所在的顶层位置；它永远不会关闭工作区。空文件夹是合法的、可持久化的状态。

文件夹的折叠是每个客户端各自的视觉状态，不会出现在任何 API 载荷中。

文件夹方法会发出 `folder.created`、`folder.updated`（重命名或成员重排）、`folder.assigned`、`folder.moved` 和 `folder.deleted` 事件。
...
文件夹事件订阅包括 `folder.created`、`folder.updated`、`folder.deleted`、`folder.assigned` 和 `folder.moved`。`folder.created` 和 `folder.updated` 包含带有序成员的文件夹记录。`folder.deleted` 包含被释放的 `workspace_ids`。`folder.assigned` 包含受影响的 `workspace_ids`（家族指派时为整个 worktree 家族）和目标 `folder_id`，顶层指派时省略 `folder_id`。`folder.moved` 包含 `folder_id` 和实际的顶层 `position`。
