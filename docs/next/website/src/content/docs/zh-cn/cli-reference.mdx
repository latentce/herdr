...
文件夹按服务器拥有的顺序对工作区分组。`folder list` 返回各文件夹及其有序成员，以及文件夹与松散工作区交错排列的顶层顺序。`folder assign` 把一个工作区移入文件夹；省略 `--folder` 则把它移回顶层。指派 worktree 家族中的任一成员会移动整个家族。位置是目标容器内从 0 开始的索引；省略则追加到末尾，超出范围则截断到末尾。`folder move` 在顶层顺序中重新定位文件夹。`folder delete` 把成员原位释放回顶层，永远不会关闭工作区。
