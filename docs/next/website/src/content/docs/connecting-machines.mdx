...
To see how responsive each machine is, set `show_machine_rtt = true` in `[ui]`. Herdr then probes every online saved machine every 3 seconds through its existing SSH connection and shows the smoothed round-trip time beside the status dot, for example `97ms ●`. The value covers the full path your input takes (local client, `ssh`, the remote bridge, and the remote Herdr server), so it is a better guide to how typing will feel than a plain network ping. The option is off by default.
