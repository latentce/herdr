...
Each online saved machine shows its smoothed round-trip time beside the status dot, for example `97ms ●`. Herdr probes the machine every 3 seconds through its existing SSH connection. The value covers the full path your input takes (local client, `ssh`, the remote bridge, and the remote Herdr server), so it is a better guide to how typing will feel than a plain network ping. Set `show_machine_rtt = false` in `[ui]` to hide it and return to probing only when the connection is quiet.
