...
If the Herdr server stops and starts again, the original pane processes are gone. Herdr restores the saved session shape: workspaces, tabs, panes, cwd, layout, and focus. Sidebar organization is part of that shape: folders, folder membership, the space order, and collapse states come back the way you left them, and stale or corrupted organization data is repaired silently on restore.
