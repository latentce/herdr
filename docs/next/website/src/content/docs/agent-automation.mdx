...
Workspaces can also be organized into folders with `herdr folder`: create and rename folders, assign workspaces (omitting `--folder` returns one to the top level), reorder the top level, and delete folders without closing anything. Assigning any worktree family member moves the whole family.
