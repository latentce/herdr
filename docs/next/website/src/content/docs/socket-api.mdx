...
| Folder | `folder.create`, `folder.list`, `folder.rename`, `folder.assign`, `folder.move`, `folder.delete` |
...
Folder methods organize the sidebar's space order. The server owns one
top-level ordered sequence interleaving folders and loose workspaces, plus an
ordered member list per folder. Every read reflects that canonical order:
`folder.list` returns the folders and the top-level order, workspace
lists follow the flattened order, and workspace records carry a `folder_id`
when the workspace is in a folder (the field is omitted for loose
workspaces).

The `herdr folder` CLI group wraps these six methods 1:1 for shells and
agents; see the [CLI reference](/docs/cli-reference/).

```json
{"id":"req_1","method":"folder.create","params":{"name":"projects"}}
{"id":"req_2","method":"folder.list","params":{}}
{"id":"req_3","method":"folder.rename","params":{"folder_id":"f1","name":"active"}}
{"id":"req_4","method":"folder.assign","params":{"workspace_id":"w2","folder_id":"f1","position":0}}
{"id":"req_5","method":"folder.move","params":{"folder_id":"f1","position":0}}
{"id":"req_6","method":"folder.delete","params":{"folder_id":"f1"}}
```

`folder.create` returns the new folder's id. Names must contain visible text;
duplicate names are allowed because the id is the identity.

`folder.assign` moves a workspace into a folder, or back to the top level when
`folder_id` is `null`. Assigning any worktree family member moves the whole
family, and the response and event report all affected workspaces. The optional
`position` is the final index of the affected block inside the target
container: the folder's member list, or the top-level order (where a folder
counts as one entry) when `folder_id` is `null`. Omitted positions append;
out-of-range positions clamp to the end. Assigning to the current container
with a position is a plain reorder inside it: a same-folder positional assign
emits `folder.updated`, and a top-level assign with `folder_id: null` plus a
position is a top-level reorder.

`folder.move` repositions a folder within the top-level order and returns the
effective position. `folder.delete` releases members back to the top level at
the folder's former position in their previous relative order; it never closes
workspaces. Empty folders are legitimate, persistable state.

Folder collapse is per-client visual state and appears in no API payload.

Folder methods emit `folder.created`, `folder.updated` (rename or member
reorder), `folder.assigned`, `folder.moved`, and `folder.deleted` events.
...
Folder event subscriptions include `folder.created`, `folder.updated`, `folder.deleted`, `folder.assigned`, and `folder.moved`. `folder.created` and `folder.updated` include the folder record with its ordered members. `folder.deleted` includes the released `workspace_ids`. `folder.assigned` includes the affected `workspace_ids` (the whole worktree family for family assigns) and the target `folder_id`, omitted for top-level assigns. `folder.moved` includes the `folder_id` and effective top-level `position`.
