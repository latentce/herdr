...
| フォルダ | `folder.create`、`folder.list`、`folder.rename`、`folder.assign`、`folder.move`、`folder.delete` |
...
フォルダのメソッドはサイドバーのスペースの並び順を整理します。サーバーは、フォルダと単独ワークスペースを交互に並べた 1 つのトップレベル順序と、フォルダごとの順序付きメンバーリストを所有します。すべての読み取りはこの正規の順序を反映します。`folder.list` はフォルダとトップレベル順序を返し、ワークスペースのリストは平坦化した順序に従い、ワークスペースがフォルダ内にある場合はワークスペースレコードに `folder_id` が含まれます (単独ワークスペースではこのフィールドは省略されます)。

`herdr folder` CLI グループは、シェルやエージェント向けにこれら 6 つのメソッドを 1:1 でラップします。[CLI リファレンス](/docs/cli-reference/) を参照してください。
...
`folder.create` は新しいフォルダの id を返します。名前には表示可能なテキストが必要です。id が識別子なので、重複した名前は許可されます。

`folder.assign` はワークスペースをフォルダに移動し、`folder_id` が `null` の場合はトップレベルに戻します。worktree ファミリーのいずれかを割り当てるとファミリー全体が移動し、レスポンスとイベントは影響を受けたすべてのワークスペースを報告します。省略可能な `position` は、対象コンテナ内での影響ブロックの最終インデックスです。対象はフォルダのメンバーリスト、または `folder_id` が `null` の場合はトップレベル順序 (フォルダは 1 エントリとして数えます) です。省略すると末尾に追加され、範囲外は末尾に丸められます。現在のコンテナに位置を指定して割り当てると、その中での単純な並べ替えになります。同じフォルダ内での位置指定の割り当ては `folder.updated` を発行し、`folder_id: null` と位置を指定したトップレベルの割り当てはトップレベルの並べ替えになります。

`folder.move` はトップレベル順序内でフォルダを並べ替え、実際の位置を返します。`folder.delete` はメンバーを以前の相対順序のまま、フォルダがあった位置でトップレベルに戻します。ワークスペースを閉じることはありません。空のフォルダは正当な、永続化される状態です。

フォルダの折りたたみはクライアントごとの見た目の状態で、API のペイロードには含まれません。

フォルダのメソッドは `folder.created`、`folder.updated` (名前変更またはメンバーの並べ替え)、`folder.assigned`、`folder.moved`、`folder.deleted` イベントを発行します。
...
フォルダのイベント購読には `folder.created`、`folder.updated`、`folder.deleted`、`folder.assigned`、`folder.moved` があります。`folder.created` と `folder.updated` は順序付きメンバーを含むフォルダレコードを含みます。`folder.deleted` は解放された `workspace_ids` を含みます。`folder.assigned` は影響を受けた `workspace_ids` (ファミリー割り当ての場合は worktree ファミリー全体) と対象の `folder_id` を含み、トップレベルへの割り当てでは `folder_id` は省略されます。`folder.moved` は `folder_id` と実際のトップレベル `position` を含みます。
