...
フォルダはサーバーが所有する並び順でワークスペースをまとめます。`folder list` は各フォルダとその順序付きメンバー、およびフォルダと単独ワークスペースを交互に並べたトップレベルの順序を返します。`folder assign` はワークスペースをフォルダに移動します。`--folder` を省略するとトップレベルに戻します。worktree ファミリーのいずれかを割り当てると、ファミリー全体が移動します。位置は対象コンテナ内の 0 始まりのインデックスです。省略すると末尾に追加され、範囲外は末尾に丸められます。`folder move` はトップレベルの順序内でフォルダを並べ替えます。`folder delete` はメンバーをその位置のままトップレベルに戻し、ワークスペースを閉じることはありません。
