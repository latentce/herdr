...
impl ClientShellState {
    ...
    /// Anchor for a navigation target hidden inside a collapsed folder, in the
    /// coordinates of a cross-endpoint `targets` list, so one `delta` step
    /// lands on the folder's visible neighbor.
    pub(super) fn hidden_folder_navigation_anchor(
        &self,
        targets: &[WorkspaceNavigationTarget],
        delta: isize,
    ) -> Option<isize> {
        let selected = self.navigate_workspace_id.as_ref()?;
        let endpoint = self
            .endpoints
            .iter()
            .find(|endpoint| endpoint.endpoint_id == selected.endpoint_id)?;
        let snapshot = endpoint.snapshot.as_deref()?;
        let empty_collapsed_groups = HashSet::new();
        let collapse = FolderCollapseState::of(&self.folder_collapse, &endpoint.endpoint_id);
        let gap = folder_hidden_gap_position(
            snapshot,
        ...
        let offset = targets
            .iter()
            .position(|target| target.endpoint_id == endpoint.endpoint_id)
            .unwrap_or(targets.len());
        let gap = (offset + gap) as isize;
        Some(if delta > 0 { gap - 1 } else { gap })
    }
...
pub(super) fn render_folder_header(
    ...
    let selected_workspace_id = selected_workspace
        .filter(|target| &target.endpoint_id == folders.endpoint_id)
