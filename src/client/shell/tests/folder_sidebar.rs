...
fn navigate_workspace_id(state: &ClientShellState) -> Option<&str> {
    state
...
fn collapsed_folder_hides_every_member_and_navigation_skips_them() {
    ...
    state.navigate_workspace_id = state.navigation_target(&ClientEndpointId::Local, "ws_b");
    state.compose(106, 24).expect("frame");
    state.handle_raw_events(vec![RawInputEvent::Key(crate::input::TerminalKey::new(
        KeyCode::Down,
        KeyModifiers::empty(),
    ))]);
    assert_eq!(navigate_workspace_id(&state), Some("ws_d"));
    state.navigate_workspace_id = state.navigation_target(&ClientEndpointId::Local, "ws_c");
    state.handle_raw_events(vec![RawInputEvent::Key(crate::input::TerminalKey::new(
        KeyCode::Up,
        KeyModifiers::empty(),
    ))]);
    assert_eq!(navigate_workspace_id(&state), Some("ws_a"));
}
